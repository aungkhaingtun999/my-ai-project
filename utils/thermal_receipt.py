...
# ==============================================================================
# PRINTER INITIALIZER (USB / NETWORK)
# ==============================================================================
...
def get_printer():
    ...
    if mode == "usb":
        if Usb is None:
            st.warning("python-escpos is not installed or USB mode is unavailable.")
            return None
        try:
            vid = shop.get("printer_vendor_id")
            pid = shop.get("printer_product_id")
            if vid and pid:
        ...
        if Network is None:
            st.warning("python-escpos is not installed or Network mode is unavailable.")
            return None
        try:
            ip = shop.get("printer_ip")
            port = int(shop.get("printer_port", 9100))
            if ip:
...
def prepare_receipt(data):
    ...
    return {
        ...
        "subtotal": num(data.get("subtotal", 0)),
        "tax_amount": num(data.get("tax_amount", 0)),
        "grand_total": num(data.get("grand_total", 0)),
        "paid": num(data.get("paid", 0)),
        "change": num(data.get("change", 0))
    }
...
def print_thermal(data):
    try:
        ...
        receipt_text += (
            shop.get("shop_name", "MY POS SYSTEM")
            + "\n"
        )
        ...
        if mode == "windows":
            ...
            if win32api:
                ...
                st.warning("pywin32 not installed")
