...
def build_session(user):
    """Build session state for authenticated user."""
...
def auth_sidebar():
    ...
    with st.sidebar:
        st.success(f"User: {user.get('full_name', 'User')}")
        ...
        if tenant_role:
            st.caption("Tenant Role: " + TENANT_ROLE_MAP.get(tenant_role, tenant_role))
